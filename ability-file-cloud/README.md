...
- agent.json — agent manifest (present in the package root). Key fields (updated):
  ...
  - "version": "0.2.0"
  - "type": "ability"
  - "abilities" and "brokers" may be declared here (see agent.json).
...
- config.toml — ability configuration (broker/local settings). See config.toml in the repo root.
...
- broker mode (npm run serve:broker): ability connects to the broker (configured in config.toml/agent.json) and processes messages via that broker.
...
- tsx used for running the TypeScript entrypoint in development
...
- tsx — run TypeScript files directly in Node.js (development)
Dev dependencies:
- typescript — compile to dist/
- @types/node — type definitions

Files of interest
- agent.json — agent manifest (entrypoint, scripts, metadata). Updated entrypoint: dist/index.js
- config.toml — runtime configuration (broker settings)
